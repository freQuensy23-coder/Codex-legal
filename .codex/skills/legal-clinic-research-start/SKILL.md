...
description: >
  Codex CLI version of legal-clinic:research-start. Research roadmap for a legal issue — statutes to check, case law areas to investigate, regulatory frameworks, Westlaw search terms. Leads and frameworks, NOT authoritative citations; students verify and develop everything. Use when a student asks where to start researching, wants a research roadmap for an issue, or needs gaps identified in existing research.
argument-hint: "[legal issue]"
---

# Codex Adaptation Notes

This skill was converted from `legal-clinic:research-start` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-clinic-research-start`, or by describing the legal task in natural language.
...
# Codex skill: legal-clinic-research-start

1. Load `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → jurisdiction, practice area.
...
`~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → jurisdiction (state), practice areas.
...
1. Read `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → `## Seed documents`. Identify any item whose purpose or filename matches the research area (e.g., "Alameda UD filing guide" for a UD habitability question; a redacted sample case file in the same practice area; a prior memo on the same issue).
