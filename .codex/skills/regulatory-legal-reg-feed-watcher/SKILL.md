...
description: >
  Codex CLI version of regulatory-legal:reg-feed-watcher. Check regulatory feeds now and report what's new since the last check, filtered by your materiality threshold. Use when the user says "check the feeds", "what's new", "regulatory update", when running from the scheduled agent, or when manually pasting a regulatory development for classification and diff.
argument-hint: "[optional: --since DATE]"
---

# Codex Adaptation Notes

This skill was converted from `regulatory-legal:reg-feed-watcher` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `regulatory-legal-reg-feed-watcher`, or by describing the legal task in natural language.
...
# Codex skill: regulatory-legal-reg-feed-watcher

1. Load `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold, feed config.
...
`~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
- **Direct regulator RSS** — fetch and parse any RSS URLs in ~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md feed
...
Each item gets a materiality tier per `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md`:
...
- **Do** log the comment deadline to `~/.codex/claude-for-legal/regulatory-legal/comment-tracker.yaml` with `item_type: ANPR` or `item_type: RFI` so the downstream tracker can distinguish these from compliance gaps.
...
- If comment tracking is enabled in ~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md: append to `~/.codex/claude-for-legal/regulatory-legal/comment-tracker.yaml`
  with status "undecided" and the default comment decision owner from ~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md
...
1. Look for `Digest output path` in `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md`. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
This skill reads the watchlist, materiality threshold, and feed configuration from `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md`. When a required value is still `[PLACEHOLDER]` or empty, say so in the output — specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run `regulatory-legal-cold-start-interview --redo` or edit `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md` and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with `regulatory-legal-cold-start-interview --redo` or by editing `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md`."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with `regulatory-legal-cold-start-interview --redo` or by editing `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md`."
...
- Change the materiality threshold. If the filter is wrong, edit ~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md.
