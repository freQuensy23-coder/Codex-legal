...
description: >
  Codex CLI version of law-student:study-plan. Build or update a long-term bar prep (or exam prep) study plan — phases, subjects weighted by weakness, daily session schedule, adaptive to session history in study-plan.yaml. Use when the user says "build a study plan", "plan my bar prep", "schedule my studying", or "how should I study for [X]".
argument-hint: "[--build | --update | --status | --cram]"
---

# Codex Adaptation Notes

This skill was converted from `law-student:study-plan` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `law-student-study-plan`, or by describing the legal task in natural language.
...
# Codex skill: law-student-study-plan

1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format, bar date, weak subjects, target study hours/day, prep course.
2. Load `~/.codex/claude-for-legal/law-student/study-plan.yaml` if it exists.
...
`~/.codex/claude-for-legal/law-student/study-plan.yaml` if it exists — extend, don't overwrite.
...
Write to `~/.codex/claude-for-legal/law-student/study-plan.yaml`:
