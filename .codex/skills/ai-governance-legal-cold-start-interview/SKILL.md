...
description: >
  Codex CLI version of ai-governance-legal:cold-start-interview. Run the cold-start interview — learns your AI governance practice and writes `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` from your AI policy, a reference impact assessment, and key vendor AI agreements. Use when the practice profile is missing or contains `[PLACEHOLDER]` markers, or when user says "set up ai governance plugin", "onboard me", "configure ai governance".
argument-hint: "[--redo | --check-integrations]"
---

# Codex Adaptation Notes

This skill was converted from `ai-governance-legal:cold-start-interview` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ai-governance-legal-cold-start-interview`, or by describing the legal task in natural language.
...
# Codex skill: ai-governance-legal-cold-start-interview

1. Check `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.codex/claude-for-legal/cache/ai-governance-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (Slack, document storage, scheduled-tasks).
...
Read `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `ai-governance-legal/CLAUDE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed.

If a CLAUDE.md exists at the old cache path `~/.codex/claude-for-legal/cache/ai-governance-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Google Drive isn't connected. In Codex CLI: Settings → Connectors → Add → Google Drive → sign in. In Codex CLI: add the Drive MCP to your config or via `/mcp`. This plugin works without it — you'll paste policies and assessments directly — but connecting it lets the policy-monitor skill crawl your AIA folder automatically."
...
`~/.codex/claude-for-legal/ai-governance-legal/ai-systems.yaml`.
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your configuration is at `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
...
6. **Before your first triage**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex CLI: authorize when a skill prompts you.
