...
description: >
  Codex CLI version of law-student:bar-prep-questions. Bar prep questions — MBE or essay, targeted at your weak subjects and bar jurisdiction. Tracks misses and comes back to patterns. Use when the user says "bar prep", "MBE questions", "practice essay", or "test me for the bar".
argument-hint: "[subject, or --mbe / --essay / --session <n>]"
---

# Codex Adaptation Notes

This skill was converted from `law-student:bar-prep-questions` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `law-student-bar-prep-questions`, or by describing the legal task in natural language.
...
# Codex skill: law-student-bar-prep-questions

1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course.
2. Also load `~/.codex/claude-for-legal/law-student/study-plan.yaml` if it exists — it tells you what subject is scheduled for today and what subtopics are still weak.
...
1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` and read the bar jurisdiction and bar date.
...
Also load `~/.codex/claude-for-legal/law-student/study-plan.yaml` if it exists (written by the `study-plan` skill). If the plan has a session scheduled for today or specifies weak subjects to weight, honor it.

## Session mode

`--session <n>` runs a focused N-question session on a specific subject, tracks performance, and writes session results back to `~/.codex/claude-for-legal/law-student/study-plan.yaml` under `session_history` so the study plan adapts.
...
If no `study-plan.yaml` exists, write session history to `~/.codex/claude-for-legal/law-student/session-history.yaml` instead so future sessions can still weight appropriately.
...
Subject distribution: weight toward weak subjects **within the subjects actually tested on the student's exam**. If `~/.codex/claude-for-legal/law-student/CLAUDE.md` says weak on Evidence and Civ Pro, 60% of questions come from those.
