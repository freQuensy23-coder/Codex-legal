...
description: >
  Codex CLI version of legal-clinic:memo. IRAC-scaffolded case analysis memo with research gaps flagged — the scaffold, not the analysis. Rule blocks are RESEARCH NEEDED, Application is STUDENT ANALYSIS prompts, Conclusion is blank. Use when a student needs to scaffold a case analysis memo, write up their analysis, or build an IRAC memo for a case.
argument-hint: "[optional: specific issue to focus]"
---

# Codex Adaptation Notes

This skill was converted from `legal-clinic:memo` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-clinic-memo`, or by describing the legal task in natural language.
...
# Codex skill: legal-clinic-memo

1. Load `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → practice areas, jurisdiction.
...
`~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → practice areas, jurisdiction, supervision style.
Intake summary and case notes for facts.

## Pedagogy check

Read the supervisor guide for this practice area at `~/.codex/claude-for-legal/legal-clinic/guides/<practice-area>.md`. Check the `pedagogy_posture` setting:
