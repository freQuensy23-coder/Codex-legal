...
description: >
  Codex CLI version of law-student:flashcards. Generate or drill flashcards for black-letter memorization — Leitner-style buckets, per-subject markdown storage, drill mode with self-assessment. Use when the user says "drill flashcards", "make flashcards from", "quiz me on cards", or wants to memorize rules.
argument-hint: "[subject] [--generate | --drill | --review | --stats | --session <n>]"
---

# Codex Adaptation Notes

This skill was converted from `law-student:flashcards` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `law-student-flashcards`, or by describing the legal task in natural language.
...
# Codex skill: law-student-flashcards

1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` → current classes, weak subjects, outline locations.
2. Apply the framework below.
3. Route by flag:
   - `--generate`: build cards from source (outline path, notes, casebook) per card-writing rules. Write to `~/.codex/claude-for-legal/law-student/flashcards/[subject]/cards.md`.
...
- `~/.codex/claude-for-legal/law-student/CLAUDE.md` → current classes, weak subjects, existing outlines
- `~/.codex/claude-for-legal/law-student/flashcards/[subject]/cards.md` if it exists (incremental build)
...
- Load `~/.codex/claude-for-legal/law-student/study-plan.yaml` if it exists and read `session_history` for this subject.
...
- If no `study-plan.yaml`, write to `~/.codex/claude-for-legal/law-student/session-history.yaml` instead.

### `--generate` — create cards

**Inputs:**
- Subject (class name or topic)
- Source (outline path, notes, or "use my existing outline from ~/.codex/claude-for-legal/law-student/CLAUDE.md")
