...
description: >
  Codex CLI version of law-student:case-brief. Brief a case in your preferred format. In drill-me mode, makes the student state the holding first. Use when the user says "brief [case]", "what's the holding in", "case brief", or pastes a case.
argument-hint: "[case name or citation, or paste the case]"
---

# Codex Adaptation Notes

This skill was converted from `law-student:case-brief` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `law-student-case-brief`, or by describing the legal task in natural language.
...
# Codex skill: law-student-case-brief

1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` → outline/brief preferences.
...
`~/.codex/claude-for-legal/law-student/CLAUDE.md` → outline/brief preferences (format, depth), learning style.
...
Per the student's format in `~/.codex/claude-for-legal/law-student/CLAUDE.md`. If none captured, default:
...
Per `~/.codex/claude-for-legal/law-student/CLAUDE.md` — some students want one-line briefs (rule + cite), some want full treatment. Match their format.
